...
func JsonInBlacklist(jwtList model.JwtBlacklist) (err error) {
	err = global.LF_DB.Create(&jwtList).Error
	return
}
...
func IsBlacklist(jwt string) bool {
	isNotFound := errors.Is(global.LF_DB.Where("jwt = ?", jwt).First(&model.JwtBlacklist{}).Error, gorm.ErrRecordNotFound)
	return !isNotFound
}
...
func GetRedisJWT(userName string) (err error, redisJWT string) {
	redisJWT, err = global.LF_REDIS.Get(userName).Result()
	return err, redisJWT
}
...
func SetRedisJWT(jwt string, userName string) (err error) {
	...
	err = global.LF_REDIS.Set(userName, jwt, timer).Err()
	return err
}
